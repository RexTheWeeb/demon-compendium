    ...
    <div class="max-w-4xl mx-auto px-6 py-10">
        ...
        <div class="bg-white shadow-lg rounded-2xl p-6 mb-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <p><span class="font-semibold text-gray-700">Origin:</span> {{ $demon->origin }}</p>
        ...
        @auth
            <div class="flex justify-between items-center">
                ...
                @if(auth()->id() === $demon->added_by || auth()->user()->isAdmin())
                    <div class="flex items-center space-x-3">
                        <a href="{{ route('demons.edit', $demon->id) }}"
                           class="bg-red-600 text-white px-4 py-2 rounded-md shadow hover:bg-yellow-600 transition">
                            Edit
                        </a>

                        <form method="POST" action="{{ route('demons.toggleVisibility', $demon->id) }}">
                            @csrf
                            @method('PATCH')
                            <button type="submit"
                                    class="px-4 py-2 rounded-md shadow text-white
                               {{ $demon->visible ? 'bg-red-600 hover:bg-red-700' : 'bg-red-600 hover:bg-red-700' }}">
                                {{ $demon->visible ? 'Hide' : 'Show' }}
                            </button>
                        </form>
